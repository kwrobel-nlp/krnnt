...
from .classes import geomean, SerialUnpickler, SerialPickler, uniq, flatten, mean, Paragraph, Sentence, Token, Form
...
class Evaluator:
    ...
    def choose_best_tag(self, pred):
        ...
        sorted_tags = sorted(out, key=lambda set_of_tags,probs,tags: geomean(probs), reverse=True)
            ...
            for j,(c,dd) in enumerate(list((zip(a,b)))[-length:]):
                ...
                if verbose:
                    ...
                    for x,y,z in sorted_tags[0:100]:
                        ...
                        if z == sample['label']:
                            print(colored(z, 'green'), y, mean(y), geomean(y))
                            break
                        else:
                            if z in sample['tags']:
                                print(z, y, mean(y), geomean(y))
                            else:
                                print(colored(z, 'yellow'), y, mean(y), geomean(y))
                    print()
